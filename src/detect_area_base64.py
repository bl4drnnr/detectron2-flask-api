...
class DetectAreaBase64(Resource):
    def post(self):
        try:
            ...
            save_encoded_string('./output/string_images/OUTPUT_BASE.txt', out_image_base)
            out_image_string = out_image_base.decode('UTF-8')

            out_partial_image_table = []
            for i in range(len(detected_rois)):
                output_image_parsed = payload["output_image_name"].split(".")[0]
                out_part_image_base = convert_image_to_base64(
                    f'./output/images/OUTPUT_PARTIAL_IMAGE_{output_image_parsed}_{i}.jpg'
                )
                
                save_encoded_string(
                    f'./output/string_images/OUTPUT_PARTIAL_IMAGE_BASE_{output_image_parsed}_{i}.txt', out_part_image_base
                )
